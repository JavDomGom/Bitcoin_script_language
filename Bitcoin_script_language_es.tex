...
\section{Transacción \textit{Coinbase}}
    ...
    \begin{figure}[H]
        ...
        \end{array}$
    \end{figure}
    En este escript se introducen 20 \textit{bytes} en la pila mediante el OP\_CODE \textbf{\texttt{OP\_PUSHBYTES\_20}}. Estos 20 \textit{bytes} se obtienen en un cálculo realizado en dos pasos. En el primer paso hay que aplicar la función \textit{hash} SHA-256 a la clave pública que corresponda en cada caso. En el segundo paso al resultado anterior hay que aplicarle la función \textit{hash} RIPEMD-160.
    \begin{figure}[H]
    \centering
        \texttt{RIPEMD160(SHA256(pubKey))}
    \end{figure}
    ...
    \centering
        ...
        $\Downarrow$ \\
        \scriptsize{SHA256(publicKey): \texttt{c12da6010de58147801cef8a508b1e3da7460002268751093c4e6aadcf0967a1}} \\
        $\Downarrow$ \\
        \scriptsize{RIPEMD160(SHA256(publicKey)): \texttt{6938581228f46c05efb6ad6f4497128f3c49f32a}}
    \end{figure}
    
    
    El resultado representa un dato que se utilizará más adelante para generar la dirección Bitcoin a la que se ha de enviar la cantidad de \textit{satoshis}. antes de añadirle un byte de versión con valor \texttt{00} y 4 bytes de \textit{checksum} por la derecha y  aplicarle el esquema de codificación Base58\footnote{https://en.wikipedia.org/wiki/Base58}.
